...
header('Cloud record listing — a read-only page per kind, refused content still visible by id');
const secondId = 'record-' + 'b'.repeat(40);
await control({ records: [record('preference'), { ...record('preference', { content: { text: '{broken' } }), memoryRecordId: secondId }], nextToken: 'page-2-token' });
const listBefore = (await calls()).length; const stateBefore = (await readdir(cloudDirectory(config))).sort().join(',');
const listed = await preferences.command('list', 'read');
const listCall = (await calls()).slice(listBefore);
assert('list issues exactly one ListMemoryRecords call scoped to the preference namespace, strategy and page size', listCall.length === 1 && listCall[0].operation === 'list-memory-records' && listCall[0].input.namespacePath === scope.preferences && listCall[0].input.memoryStrategyId === config.preferenceStrategyId && listCall[0].input.maxResults === 32 && listCall[0].input.nextToken === undefined);
assert('rows carry id, time and a one-line preview; refused content is listed by id without its text', listed.includes(`1. ${id} · 2026-01-01T00:00:00.000Z · [{"language":"English"`) && listed.includes(`2. ${secondId} · 2026-01-01T00:00:00.000Z · content refused by bounded validation (not shown)`) && !listed.includes('{broken'));
assert('the header states counts, omissions and the next page command', listed.includes('2 listed (page of 32), 0 other-kind omitted, 1 content refused · more: yes — /cloud-memory list preferences after page-2-token'));
assert('listing writes nothing locally and adopts nothing', (await readdir(cloudDirectory(config))).sort().join(',') === stateBefore && await preferences.context() === '');
await control({ records: [record('episode')] });
const paged = await preferences.command('list episodes after page-2-token', 'read'); const pagedCall = (await calls()).at(-1);
assert('a kind and page token are passed through; the episode namespace path is the sessions prefix', pagedCall.operation === 'list-memory-records' && pagedCall.input.nextToken === 'page-2-token' && pagedCall.input.namespacePath === scope.episodes && pagedCall.input.memoryStrategyId === config.episodicStrategyId && paged.includes('episodes · namespace') && paged.includes('1 listed') && paged.includes('more: no'));
assert('episode previews escape markup and stay one bounded row', paged.includes('‹episode›‹intent›Fix tests‹/intent›') && paged.split('\n').every((line) => [...line].length <= 400));
await control({ records: [] });
assert('an empty page says so', (await preferences.command('list reflections', 'read')).includes('(none)'));
await control({ records: [record('preference', { namespaces: ['/users/wrong/'] })] });
const mismatch = await preferences.commandResult('list', 'read');
assert('a scope mismatch refuses the whole page before any content is decoded', !mismatch.ok && mismatch.text.includes('namespace mismatch') && !mismatch.text.includes('concise'));
const unknownKind = await preferences.commandResult('list other', 'read');
assert('an unknown kind is refused locally with the usage line', !unknownKind.ok && unknownKind.text.includes('usage:'));
await control({ records: [record('preference')] });
