...
async function capture(memory: CloudMemory, turn: number, options: { failed?: boolean; partial?: boolean; goal?: string; noAction?: boolean } = {}) {
  ...
  observer.end(); memory.settle({ ...closeTurn(memory.session, turn), partial: options.partial ?? false });
...
const late = new UploadTurn(2, ''); add(late);
late.actions[0]!.original = { turn: 1, ordinal: 1, invocation: 'call', invocationScope: 1, tool: 'arbitrary-tool' };
...
const ackOnly = new UploadTurn(1, 'goal'); add(ackOnly); ackOnly.actions[0]!.result = { missing: 'Background acknowledged; final result pending' };
...
const damaged = new UploadTurn(1, 'goal'); add(damaged); damaged.actions[0]!.input.losses.push({ path: '', reason: 'Accessor or absent own value omitted without evaluation' });
...
concurrentPublication.begin(1, 'goal while network reserved'); concurrentPublication.settle(closeTurn('concurrent-publication', 1));
...
await preEnable.command('auto', 'user'); beforeRace = calls.length; preEnable.settle(closeTurn('pre-enable', 1));
