...
async function missingDirectory(): Promise<void> {
  ...
  assert('developer offers evidence-selected small/normal/complex presets', ['`small`', '`normal`', '`complex`', '50 / 100', '140 / 240', '240 / 400', '15 / 30', '40 / 80', '80 / 120'].every((term) => workflow.includes(term)));
  ...
  assert('developer enables process-only offload and uses the whole-worker hard value', workflow.includes('`--yolo --context-offload --max-model-calls <worker-hard>`'));
