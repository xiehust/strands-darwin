...
async function missingDirectory(): Promise<void> {
  ...
  assert('developer offers evidence-selected small/normal/complex presets', ['`small`', '`normal`', '`complex`', '10 / 20', '20 / 40', '40 / 80', '40 / 80', '120 / 200', '200 / 320', '15 / 30', '40 / 80', '80 / 120'].every((term) => workflow.includes(term)));
  assert('developer distinguishes soft targets from hard CLI ceilings', workflow.includes('**soft target**') && workflow.includes('**hard ceiling**') && workflow.includes('never put the soft value there'));
  assert('developer requires an 80% checkpoint and reports the selected preset', workflow.includes('At 80% of the hard ceiling') && workflow.includes('state it to the user') && workflow.includes('selected preset'));
  assert('developer enables process-only offload and uses preset hard values', workflow.includes('`--yolo --context-offload --max-model-calls <planning-hard>`'));
  assert('developer compacts before bounded implementation', workflow.includes('--compact-before --max-model-calls <implementation-hard>') && workflow.includes('compacts the planning transcript'));
  assert('developer bounds corrections and compacts only large prior turns', workflow.includes('--max-model-calls <correction-hard>') && workflow.includes('crossed its soft target substantially') && workflow.includes('narrow correction'));
  ...
  assert('developer requires explicit same-session continuation', workflow.includes('`--session <captured-id> --yolo --context-offload --compact-before') && workflow.includes('Never use `--continue` or `--resume`'));
