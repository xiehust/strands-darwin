...
export class MemoryTransport {
  ...
  private requests = new Map<AbortSignal, { check: () => void; raw?: unknown; beforeLaunch?: BeforeLaunch }>();
      ...
      extensions: [{ configure: extension => {
        ...
        extension.setHttpHandler({
          ...
          handle: async (request, options) => {
            ...
            let guard: RequestLaunch | void = undefined;
            if (state.beforeLaunch) {
              ...
              guard = await state.beforeLaunch(bytes);
            }
            const start = () => { state.check(); return handler.handle(request, options); };
            const pending = guard ? (await guard(start)).pending : start();
  ...
  async call<K extends MemoryOperation>(operation: K, input: MemoryInputs[K], signal?: AbortSignal, beforeLaunch?: BeforeLaunch): Promise<unknown> {
    ...
    const state: { check: () => void; raw?: unknown; beforeLaunch?: BeforeLaunch } = { ...(beforeLaunch ? { beforeLaunch } : {}), check: () => {
