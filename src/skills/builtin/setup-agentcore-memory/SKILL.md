...
The effective upload mode may come from `projectOverrides[projectId].agentCoreMemory` in the global config. Preserve that entire map and all other projects during any approved config repair. Identity uses global explicit `agentCoreMemory.projectId`, otherwise SHA256 of Darwin's canonical project key; overrides cannot change region/resource/actor/project/strategies. Defaults < global < current project. Only registered upload mode and daily budgets are overridable; never add unknown fields.
