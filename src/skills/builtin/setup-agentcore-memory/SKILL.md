...
## Project upload authority (preserve during setup)

The effective upload mode may come from `projectOverrides[localKey].agentCoreMemory` in the global config. Preserve that entire map and all other projects during any approved config repair. The local key is always SHA256 of Darwin's canonical project key; explicit `agentCoreMemory.projectId` selects only the cloud namespace/shared quota, never consent. Status and doctor label both identities. Auto authorization v2 additionally binds the local key; v1 needs user reconfirmation, never silent migration. Overrides cannot change region/resource/actor/project/strategies. Defaults < global < current project. Only registered upload mode and daily budgets are overridable; never add unknown fields.

Do NOT enable auto, construct its authorization, drive a pseudo-terminal, or run cleanup on the user's behalf. After setup the user may type `/cloud-memory auto` in the idle TUI: this is consent for NEW turns in this project, captures a fresh scope-bound epoch, and warns that content can include secrets. `/cloud-memory manual` persists revocation immediately without resetting conversation or deleting evidence. Root `upload: auto` alone is not authorization. Defaults are 500 attempts/104857600 bytes per UTC day; configurable project budgets are positive bounded integers. Auto only sends normally closed traceable v2 events; tool failures do not mean ineligible, and endTurn does not mean goal success. Cancelled/incomplete/seriously damaged collections stay manual. Old pending is never backfilled. Only auto-accepted local bodies expire after seven days; receipts remain, manual/pending/failure/cloud data do not expire. No automatic preference adoption or cloud deletion.

User-only `/cloud-memory discard-legacy` previews unaccepted current-project/current-binding legacy local events; the user must submit the displayed manifest hash to clean them. Never execute cleanup as part of setup. Standalone CLI remains read-only and no mutation tool is exposed.
...
Confirmed choices override all example defaults, not only the capitalized placeholders: use the confirmed `region`, `upload` (including `off`), `preferences` (including `false`), and `timeoutMs`. Preserve those choices instead of resetting them to `manual`, `true`, or `5000`; validate the exact merged config against the confirmed settings before writing. Memory ID must be an actual returned ID; the two strategy IDs must be distinct and use the actor segment grammar above. Do not set a global `projectId` default: omission derives automatic isolation from the canonical project key. Only when deliberately sharing checkouts, confirm explicit `projectId` matching `[a-z0-9][a-z0-9_-]{0,63}` (lowercase, 1–64 characters). This config camelCase field maps to lowercase AWS namespace variable `projectid`. Timeout accepts 100–15000 ms. Setup writes support `off` or `manual`; proposal `manual` differs from the runtime's default `off`. Project auto is available only through the user's separate `/cloud-memory auto` authority, never this setup writer. Unknown config fields are rejected.
...
The manual projection contains bounded literal **USER goals**, **TOOL evidence**, and **OTHER source/outcome/omissions**. Every public textual tool argument/result is eligible, including arbitrary MCP tools, free-form logs, file/diff content, skills, memory results and public subagent reports. It excludes all assistant prose/reasoning and binary/images; it never traverses child conversations, reads tool paths or hydrates offload/archive content. Execution evidence is captured before after-tool hooks, not necessarily as finally shown to the model; observed public fallback results are labelled when no execution snapshot exists. This is not a confidentiality guarantee: tool content can include secrets. No automatic uploads, history backfill, test writes or preference adoption belong to setup.
