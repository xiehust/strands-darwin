...
- **when selecting** — a backlog row scoring below the gate is set to `abandoned` with the reason `below score gate (Score = <n> < 6)` and skipped. Continue the loop with the next direction; a gated row never halts the batch.
...
3. **Mark the work.** Change exactly the one selected direction to `in-progress`.
...
- **the batch is exhausted**: every row is `done` or `abandoned`. Normal completion.
...
- **continuing is not worth it**: every remaining row is below the gate, obsolete, or already covered by shipped work. Record each as `abandoned` with that reason.
