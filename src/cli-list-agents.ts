...
export async function runListAgentsCli(projectRoot: string, args: readonly string[]): Promise<void> {
  if (args.length > 0) {
    process.stderr.write(usageErrorText('list-agents takes no arguments. Usage: darwin list-agents'));
    process.exitCode = 2;
    return;
  }
  process.stdout.write(`${formatLocalAgents(await readLocalAgents(projectRoot))}\n`);
}
