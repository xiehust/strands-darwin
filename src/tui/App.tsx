...
import { isRefusalStop, refusalNoticeWithRewind } from '../agent/refusal.js';
        ...
        await runWithStreamResumption(
          ...
          async (turnInput) => {
            ...
            for await (const event of runtime.send(
              ...
              // with the stop reason actually received (SRF-029) and the remedy only
              // this driver has: the runtime catalogued the prompt, so `/rewind` to it
              // removes the declined exchange (SRF-030).
              if (event.type === 'agentResultEvent' && isRefusalStop(event.result.stopReason)) {
                dispatch({ type: 'notice', text: refusalNoticeWithRewind(event.result.stopReason), severity: 'warn' });
              }
            }
          },
          (error) => {
