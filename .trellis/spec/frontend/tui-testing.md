...
- `Escape` dismisses the current completion query or active recall walk without editing/submitting the draft. A second Escape is inert; a changed query re-arms completion. Permission denial and compaction ownership remain earlier in input precedence.
