...
- Background lifecycle `bash` calls (`start`, `list`, `status`, `output`, `wait`, `stop`) are a presentation-only projection. Compact mode suppresses successful status, empty output polls, and valid empty-running waits; a wait with output retains only its incremental child text, while an empty terminal wait retains one short-id/state row. Nested status/command/path/cursor metadata stays out of compact history. Failures remain fully diagnostic, expanded mode keeps the ordinary bounded payload, and unknown or internally contradictory successful payloads fall back rather than being silently suppressed.
