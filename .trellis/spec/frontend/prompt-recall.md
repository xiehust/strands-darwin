...
**Esc Esc shortcut (SER-059).** The chooser has exactly one opener, `App.tsx`'s `openRewindChooser()` — the `/rewind` command body from the `startRewind` guard through `setRewindSearch(openRewindSearch(...))` — and the Escape chord calls that same callback, so notices, capacity warning, acceptance and the returned-unsent prompt cannot diverge. An Escape that no owner consumed (permission denial, rewind search, history search, completion menu and recall all return earlier and keep their key) on an **empty idle composer** — `status === 'idle'`, empty queue, empty draft text — arms a timestamp ref and does nothing visible; a second such Escape within `ESCAPE_REWIND_CHORD_MS` (500 ms, `src/tui/rewind-search.ts`) opens the chooser, and `\x1b\x1b` delivered as one Ink `escape`+`meta` event counts as both presses. The key handler clears the armed timestamp on *every* key first and only the composer branch re-arms it, so a consumed Escape, any other key, a draft, a running turn or `!` command, a queued entry or a pending permission ends the chord; two Escapes further apart than the window re-arm rather than open. The chord dispatches no `userInput` row, writes no trajectory record, adds no frame row, uses no timer and no runtime accessor beyond `listRewindCheckpoints()`; with no catalogue it shows exactly the `/rewind` notice. `openRewindChooser()` resolves `true` only when the chooser is up; on any refusal (`false`) the `/rewind` command clears its own text from the draft like every other consumed local command — before this, a refused `/rewind` lingered in the editor and the next submission was glued onto it. `/help`, `docs/user-guide/reference.md` and README state the key. pty check: `spike/verify-tui.ts escRewind` (free; two separate Escape pty events, a real gap, the same anchors as `rewind`).
