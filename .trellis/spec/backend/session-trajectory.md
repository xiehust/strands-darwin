...
| `modelCall` | one record per **completed** model call of the turn: `attempt` (the SDK's 1-indexed `attemptCount`, so retries stay visible as gaps), `ms` since turn start, `stopReason` when reported, `contextTokens` when the SDK reported `projectedInputTokens` for the call, and `spend` — that one call's counters in the same bucket vocabulary as `turnEnded.spend`, produced by a per-call projector injected beside the turn meter (trajectory code still imports no Agent/Model/config). A failed attempt (no `stopData`) writes nothing; the raw `afterModelCallEvent` stays a *counted drop* in `turnEnded.dropped`, because this is a derived projection like `spend`/`partialText`, not a fifth recorded event type. Absent fields are unknown, never 0. Replay prints one bounded line per call; readers that predate the type skip it (additive under the current schema version) |
