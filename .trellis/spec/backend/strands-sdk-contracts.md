...
## Scenario: explicit project memory search (`SER-031`)

### 1. Scope / Trigger

Use this contract when adding or changing model access to project trajectory history. Retrieval is
explicit only: the model calls one ordinary tool while handling a turn.

### 2. Signatures

```text
search_memory({ query: string }) -> bounded text tool result
createSearchMemoryTool(projectRoot, activeSessionId, agentId)
searchTrajectories(projectRoot, query, agentId,
  { excludeSessionId, sessionLimit: 20, limit: 10 })
```

### 3. Contracts

The tool performs case-insensitive literal retrieval over existing project trajectory files and
returns source-labelled session/turn/type excerpts through ordinary tool-use/result events. The
active session is excluded by default. `AgentRuntime.create` registers it before the existing child
tool snapshot, so default children inherit it and explicit child allowlists remain authoritative.

It is not an alternate Agent loop or ambient context channel: no vector/embedding provider,
generated summary, derived index, memory database, startup injection, system-prompt mutation or
post-turn mining exists. `classify('search_memory', input)` returns `read`, including plan mode;
unknown tools retain fail-closed `execute`. Shared tool rendering is the only presentation path.

### 4. Validation & Error Matrix

| Condition | Result |
|---|---|
| blank or >256-code-point query | SDK tool validation error before trajectory reads |
| active session contains query | excluded; never returned as a source |
| no prior match | successful bounded result states no matches |
| damaged / snapshot-only session | damage / missing record stated; readable sessions still searched |
| session, hit or result bound crossed | omission/cut stated; caller can refine the literal query |
| unknown tool name | remains `execute`, never inherits `search_memory` safety |

### 5. Good / Base / Bad Cases

- **Good:** model explicitly searches a prior error phrase; result names session, turn and type.
- **Base:** no match or old snapshot without a record is stated without writing any session bytes.
- **Bad:** preloading trajectory matches into the system prompt, generating durable memories, or
  indexing embeddings creates an unrequested ambient/second-store path and is forbidden.

### 6. Tests Required

`spike/verify-memory-tool.ts` seeds real trajectory/snapshot damage and limits, invokes the actual
SDK tool offline, hashes every source and pointer, and structurally excludes model/network/writer/
prompt/UI paths. `verify-subagents.ts` proves default inheritance and explicit allowlisting;
`verify-permission-modes.ts` proves read safety while unknown tools fail closed.

### 7. Wrong vs Correct

```typescript
// WRONG: hidden ambient recall and a second retrieval system.
agent.systemPrompt += await generatedVectorMemory(projectRoot)

// CORRECT: one visible bounded tool over the existing literal reader.
tools: [...tools, createSearchMemoryTool(projectRoot, sessionId, AGENT_ID)]
```

Full bounds and record honesty rules: `.trellis/spec/backend/session-trajectory.md` § Explicit model
reader.
