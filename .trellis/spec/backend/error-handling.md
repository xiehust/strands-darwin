...
| `search_memory` query blank/over 256 code points | SDK tool validation error; read nothing | Model-controlled retrieval input must be bounded before filesystem work |
| `search_memory` encounters damaged or snapshot-only sessions | Return bounded ordinary tool text naming damage/missing records and continue over readable prior sessions | Historical absence/damage is retrieval state, not a reason to hide other matches; source bytes remain untouched |
