...
must fix in configuration — invalid `config.json` values, missing API key env vars,
malformed `.mcp.json`. `src/cli.ts` catches it before Ink mounts and prints a plain
...
|---------|----------|-----|
| `config.json` invalid value / missing key env | `ConfigError`, refuse to start | Agent would misbehave silently |
| `.mcp.json` malformed | `ConfigError`, refuse to start | User asked for MCP and got none |
