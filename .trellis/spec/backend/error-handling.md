...
| Background `wait` is malformed, times out, is cancelled, or sees shutdown | Require integer `waitMs` 1–30000; return bounded reason/status/incremental output; cancellation observes only, shutdown proceeds to reaping | A monitoring convenience must neither hang the tool loop nor become a second command/ownership channel |
