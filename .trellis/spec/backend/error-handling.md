...
| Learned-memory config `memory` is non-boolean, placed inside a model entry, or explicitly `true` with `trajectory: false` | `ConfigError`, refuse to start | Explicit intent must never be guessed or silently ignored; omitted memory with `trajectory: false` is the defined implicit opt-out |
| Learned-memory source read, extraction, timeout, queue pressure, or store write fails | Completed turn remains successful; scheduler latches one bounded `memoryStatus.problem`, existing post-turn/headless surfaces warn; effective `memory: false` creates nothing | Derived memory is advisory and must never become a second turn failure path |
| Generated-memory anchor is absent, unsafe, unreadable, binary/oversized, changed/deleted, or expired | Record/report exact bounded `unknown`/`invalid`/`expired` reason and omit generated entry from ambient prompt; preserve explicit user notes; runtime continues | Learned context is advisory, so uncertainty must narrow prompt context rather than become trust or a startup/model failure |
