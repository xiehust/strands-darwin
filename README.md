...
- **Safe approval modes:** `default`, classifier-assisted `auto`, read-only `plan`, and explicit `yolo`, with project-scoped allow rules reviewed in full before saving and revocable in-session, plus deny rules that hold in every mode, `yolo` included. A checkout's own hook commands, MCP servers and legacy allow rules stay held until you accept one [workspace-trust](docs/user-guide/permissions.md#workspace-trust) modal listing exactly what they would run; the answer lives outside the repository, and headless runs never ask.
...
- **Extension layers:** built-in and project/global skills, custom commands, child agents with workflow-DAG delegation, tool hooks, and stdio/HTTP MCP servers, with portable `.agents/` discovery. Custom commands replace `$ARGUMENTS`; without it, nonempty trimmed arguments follow the untouched template and two newline characters. See [custom commands](docs/user-guide/extensions.md#custom-commands).
...
/review --commit <SHA>  review an exact 40-hex commit
...
`/review` asks for prioritized, evidence-backed bugs and separate test gaps in staged/unstaged changes and relevant untracked files. An optional focus stays literal text. `/review --commit <40-hex-SHA>` instead asks for the exact commit's parent diff (or the empty tree for a root commit), surrounding code and tests; malformed explicit scopes show local usage, and missing objects must be reported honestly. It asks for no edits or commits unless separately requested, but **does not enforce read-only mode** or change permissions. The built-in now owns `/review`; rename a custom command or skill using that slash name (for example, to `audit`). See [reviewing changes](docs/user-guide/using-darwin.md#reviewing-changes).
...
Run **`/setup-agentcore-memory`** in the TUI for guided setup. Darwin reads its bundled guide and first checks existing `~/.darwin/config.json` through the permission gate, then uses offline `darwin doctor`, local `darwin cloud-memory status`, and a bounded SDK read via `darwin cloud-memory preferences`. Healthy existing memory needs no repeat setup, actor question or restart; empty records count as a successful read, not proof of extraction/write access. Only missing/disabled setup needs actor/default questions; failed or blocked checks need targeted repair confirmation, never an automatic reset. Arguments do not authorize changes. Prefer a compatible existing Memory; no uploads or preference adoption are authorized by setup. Runtime reads/writes use the pinned official `@aws-sdk/client-bedrock-agentcore` client, not an AWS CLI executable. Legacy `cliPath` is validated but ignored with a migration notice. The optional `agentcore` infrastructure CLI is separate; importing the existing resource remains deferred because its schema/import mapping cannot preserve custom namespace keys/templates. See the [English guide](docs/user-guide/agentcore-memory.md) for exact templates, config, IAM, `/cloud-memory` commands, privacy limits and opt-in synthetic verification. No live Memory service test was run for this implementation.
