...
| Workflow DAG tool | Parent-only `workflow`: bounded data-only DAG (≤8 nodes) refused with bounded errors before any child/model/dispatch exists; execution is the SDK `Graph` (AND scheduling, dependency merge, cancel signal into unstarted nodes) — never reimplemented; every node built by the shared `buildRecipeChild` recipe with its own dispatch-registry entry; only terminus content returns, child transcripts/trajectory stay private; never in child catalogues; reads-parallel/writes-serialized stated in the description | `src/agents/workflow-tool.ts`, `src/agents/child-recipe.ts` | `backend/strands-sdk-contracts.md`; `verify-workflow-tool.ts`† |
