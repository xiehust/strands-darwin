...
| Learned project memory | `memory: true` only; delayed bounded derivation from clean durable turns plus explicit user-only `/memory`; strict project-keyed state carries provenance, unvalidated freshness and heuristic sensitivity state; forget suppresses generated ids durably and refreshes the live prompt before return; no model tool/vectors | `src/memory/` | doc §; `verify-memory.ts`†, `verify-memory-command.ts`†, `tui completion` |
