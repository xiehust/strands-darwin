...
| Learned project memory | `memory: true` only; delayed/coalesced/timeout-bound derivation from clean closed successful durable turns into bounded project-scoped Markdown with provenance; secret/dump/reasoning/tool material excluded; only labelled fallible index is prompt-loaded; no model tool/vectors | `src/memory/` | doc §; `verify-memory.ts`† |
