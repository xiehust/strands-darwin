...
| System prompt composition | Fixed order: base → `<project-instructions>` → `<available_skills>` → optional `<learned-memory>` → `<working-context>` → cache point; catalogue/memory never duplicate, current context re-derived every run | prompt modules | doc § |
| Learned project memory | On by default when trajectory is available; `memory: false` opts out, omitted memory follows `trajectory: false`, and explicit `memory: true` + `trajectory: false` is invalid; delayed bounded derivation plus explicit user-only `/memory`; generated facts require exact current project-relative text anchors and expire at configured 28-day default (0 disables age only); unknown/invalid/expired generated context omitted before every request; user notes never auto-expire/validate; no model tool/vectors | `src/memory/` | doc §; `verify-memory.ts`†, `verify-memory-command.ts`†, `verify-memory-validation.ts`†, `tui completion` |
