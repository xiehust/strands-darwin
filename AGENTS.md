...
| Process exit | Each runtime reaps its own persistent shell (`retire()` on `/clear`); background jobs reaped as process groups (TERM→KILL); their bounded `wait` observes cancel/shutdown and shares the output cursor; unref'd 500ms exit fallback after shutdown | `runtime.shutdown()`, `cli.ts` | `verify-background-bash.ts`, `probe-cancel-exit.ts`, `verify-clear-session.ts`†, `tui bashExit` / `cancelThenContinue` (*live*) |
