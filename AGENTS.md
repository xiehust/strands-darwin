...
| `/compact <focus>` — the SDK default prompt plus one bounded section, built per call | Manager built per `compact(focus?)` call (focus is SDK constructor-only); unfocused config is exactly `{ summaryRatio: 0.8, preserveRecentMessages }` so the SDK default prompt applies unchanged (`--compact-before` always unfocused); focus trimmed, ≤400 code points, over-cap is a local notice before hook/model; prompt = root-imported `DEFAULT_SUMMARIZATION_PROMPT` + one fixed section (pinned patch re-exports it from `dist/src/index.*` — never a copied string or deep import); hook payloads unchanged | `src/agent/compact.ts`, `AgentRuntime.compact`, `src/tui/App.tsx` | `backend/strands-sdk-contracts.md`; `verify-compact.ts`†, `tui completion`, `tui compacting` (*live*) |
