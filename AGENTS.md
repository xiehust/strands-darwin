...
| Session trajectory and explicit memory search | Observer except one explicit reader: bounded `userInput` barrier is durable before invocation and degrades open; later observation stays synchronous/no-I/O/non-throwing; bytes never rewritten; `search_memory` is bounded literal prior-session retrieval, excludes active session, writes/indexes/injects nothing, uses ordinary visible tool lifecycle, and follows existing child allowlists | `src/trajectory/`, `AgentRuntime.send` | `backend/session-trajectory.md`, `backend/strands-sdk-contracts.md`; `verify-trajectory.ts`†, `verify-memory-tool.ts`† |
