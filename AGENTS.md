...
| Completion guard — one private driver-owned continuation | A bounded conservative classifier withholds a successful parent `endTurn` internal action note; tool-bearing note candidates fail open. Separately, a latest successful parent `update_plan` with unfinished items may accept all first-turn events and receive one fixed unrecorded continuation; direct user blockers, errors, cancellation, a second gap, or continuation failure never loop | `src/agent/completion-guard.ts`, `AgentRuntime.beginCompletionGuardTurn`, TUI/headless drivers | same backend specs + `frontend/live-frame.md`; `verify-completion-guard.ts`† |
