...
| `/rewind` — SDK checkpoint branch only | Pre-invocation immutable SDK snapshots; only completed bounded prompts catalogued; fresh successor restores selected source snapshot, then current memory/working-context/cache; source/pointer/workspace untouched; selected prompt returns unsent; omission notice names files, shell/`!`, hooks, MCP writes, subagents, jobs and learned memory | `src/agent/rewind.ts`, `AgentRuntime.startRewind`, `src/tui/rewind-search.ts` | `backend/strands-sdk-contracts.md`, `backend/session-trajectory.md`, `frontend/prompt-recall.md`; `verify-rewind.ts`†, `verify-rewind-search.ts`†, `tui rewind` |
