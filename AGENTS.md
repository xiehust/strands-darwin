...
| --- | --- | --- | --- |
| SDK reuse — the agent loop is never forked | Only `runtime.ts` constructs `Agent`, as a thin assembly; customize via SDK extension points (interventions, plugins, conversation manager), never by intercepting the loop | `src/agent/runtime.ts` | doc § |
...
| Stream interruption — one driver-owned continuation | Only exact `ModelError: Stream ended without completing a message` gets one visible successor turn; original error/failed record stay intact, continuation uses bounded private anti-repeat input through ordinary orchestration, never runtime/SDK-loop retry | `src/agent/stream-resumption.ts`, TUI/headless drivers | `verify-stream-resumption.ts`†, `verify-headless-structured.ts`† |
...
| `/rewind` — SDK checkpoint branch only | Hard cap 100 rewind-owned immutable snapshots via bounded public listings (failures count; full cap skips capture, not turn); only completed bounded prompts catalogued; fresh successor restores selected source snapshot, then current memory/working-context/cache; source/pointer/workspace untouched; selected prompt returns unsent; omission notice names files, shell/`!`, hooks, MCP writes, subagents, jobs and learned memory | `src/agent/rewind.ts`, `AgentRuntime.startRewind`, `src/tui/rewind-search.ts` | `verify-rewind.ts`†, `verify-rewind-search.ts`†, `tui rewind` |
...
| Agent-managed project memory | Default-on with trajectory; parent-only `memory_recall` is safe local validated lexical read, `memory_save` is an un-ruleable ordinary write staged until matching durable successful `endTurn`; exact project-line or current-user-quote evidence, v3 atomic state/migration/suppression/expiry, no ambient archive, vector/network/second model call, children isolated | `src/memory/`, `src/agent/runtime.ts`, `src/trajectory/writer.ts` | `verify-memory.ts`†, `verify-memory-tools.ts`†, `verify-memory-command.ts`†, `verify-memory-validation.ts`† |
...
| Thinking effort | Always `adaptive` (`output_config.effort`, never nested in `thinking`, never `budget_tokens`); unsupported levels clamped and reported, never sent; `/effort` uses `Model.updateConfig()` — the conversation survives | `src/agent/thinking.ts` | `verify-thinking-live.ts` (*live*) |
| Subagents | Never set `toolExecutor` (SDK already races one message's calls); permissions carry `source`; bounded ≤30s heartbeats expose only id/elapsed/safe phase on existing rows/headless progress; `/agents cancel <id>` is user-only and targets one child; timers/cancellers clear before settlement; records never carry child transcript; parallelism is for **reads**; running dispatches (incl. `workflow` nodes) capped by `maxConcurrentSubagents` (default 8), refused before model/child, settlement frees; the report crosses through one pure projection (`report-projection.ts`): framing-tag/`Human:`/`Assistant:` imitation lines gain one backslash plus one marker line, never removed/reworded, clean input byte-identical; a child failing after text rethrows original + fixed note + ≤4000-cp last assistant text through that projection, still `failed`, cancellation never wrapped; background delegation is the SDK's own `backgroundTasks` option (`agentic` = `subagent`/`workflow`, gate before route, result delivered in the same turn), its hook-only after-event forwarded by `BackgroundDelegationObserver` — never a darwin scheduler | `src/agents/subagent-tool.ts`, `src/agents/dispatch-registry.ts`, `src/agent/background-delegation.ts` | `verify-subagent-heartbeats.ts`†, `verify-subagent-limit.ts`†, `verify-report-projection.ts`†, `verify-failed-child-text.ts`†, `verify-background-delegation.ts`†, `verify-subagents.ts` (*live*) |
...
| Process exit and persistent foreground cwd | Each runtime reaps its own persistent shell (`retire()` on `/clear`); foreground calls serialize per Agent and report effective cwd; only narrow simple cwd-missing/project-root-existing relative paths are refused before launch; exit 0 returns captured output + restart notice, nonzero/signal stays failure; foreground children read `/dev/null` (prompts get EOF at once, sentinels unreachable), `stop()` kills the shell's process group, and `cancelSignal` (Esc / headless Ctrl+C) kills the running command the same way; background jobs reaped as process groups (TERM→KILL); bounded `wait` shares the cursor, with opt-in terminal-focused aggregation up to five minutes plus running-timeout wait-again guidance; unref'd 500ms exit fallback | `runtime.shutdown()`, SDK patch, `cli.ts` | `verify-background-bash.ts`†, `probe-cancel-exit.ts`, `verify-clear-session.ts`†, `tui bashExit` / `cancelThenContinue` (*live*) |
