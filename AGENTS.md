...
| SDK HTTP request — parent-only ordinary gated tool | Parent runtime directly registers the SDK `httpRequest` singleton (`http_request`) in its ordinary tools list; it remains unknown/fail-closed `execute`, so hooks/permission/plan denial precede the callback; never add it to child catalogues or wrap/bypass SDK execution | `src/agent/runtime.ts`, `src/agent/permission.ts` | `backend/strands-sdk-contracts.md`; `verify-http-request-tool.ts`† |
