...
| File-edit diffs | Diff of the tool *input*, never read from disk; `- `/`+ `/`  ` markers survive ANSI stripping and reconstruct old/new exactly; approving writes the untruncated input; tone scoped to `fileEditor`; compact rows show a bounded excerpt explicit about what it withheld; `+N -N` stat and intraline bold are marker-derived enhancements — never in `summary`, which replay prints verbatim | `src/tui/edit-diff.ts` | `frontend/tui-testing.md`; `verify-edit-diff.ts`†, `verify-visual-language.tsx`† |
