...
A cloned repository is untrusted input, and three things a checkout can carry would otherwise *run or grant* the moment darwin starts: hook command files (`.darwin/hooks.json`, `.darwin/hooks/*.json`, `.agents/hooks.json`, `.agents/hooks/*.json`, or a `hooks` key in a committed `.darwin/config.json`), MCP servers declared in `.darwin/mcp.json` or the root `.mcp.json` (every stdio entry is a process spawned before your first prompt), and the legacy `permissionRules` of a committed `.darwin/config.json`. Since SER-090 none of them is armed until you say so.

On the first interactive launch in a project whose checkout declares any of these, darwin shows one modal **before** the runtime exists — before a hook could run, a server could spawn or a rule could apply. It names the project root and lists exactly what the checkout would arm: each hook file with its dialect and per-event command counts, each MCP server with its command and arguments (or URL) and the file that declares it, and the legacy rule file with its allow/deny counts. An unparseable hook or MCP file is listed as unreadable — still something the checkout carries. `y` (or Enter) accepts; `n` declines; Escape declines for this session only and stores nothing, so the next launch asks again.

The answer is stored in `~/.darwin/projects/<key>/trust.json` as `{ "trusted": true|false, "decidedAt": "<ISO instant>" }` — the same user-owned project directory as `permission-rules.json`, outside the repository, so a committed file cannot grant a clone trust in itself (a `.darwin/trust.json` inside the checkout is never read). A stored answer is applied silently on every later launch; delete the file to be asked again. A malformed file reads as "no decision" with a bounded notice, never a crash.

Declining (or Escape) starts the session with the repository-supplied layers **held back**: no hook command from those files runs, no project MCP server is spawned, no legacy project allow or deny rule is granted, and a permission prompt's "always allow" cannot copy the committed rules into your user-owned file. One transcript notice states the omission; `/status` adds ` · N held (untrusted project): …` to its `mcp` and `hooks` rows; `/mcp` lists each held server as `held (untrusted project)` with no connection attempted. A `/clear` or `/rewind` successor inherits the decision. Everything user-owned still loads as before — `~/.darwin` and `~/.agents` hooks, `~/.darwin/mcp.json`, your `permission-rules.json` — and skills, custom commands and `AGENTS.md` are prompt content, not execution, so they are never part of the gate. A project that declares none of these files sees nothing new.

Headless (`-p`) never shows a dialog. A stored `trusted: true` applies; with no decision or `trusted: false` the layers are held back, text output writes one `trust:` stderr line naming what was held, and structured output carries `run.started.trust` (`{ "state": "trusted"|"untrusted"|"undecided", "held": [...] }`). Trust is keyed on the project root like every other darwin path, not on git state; it adds no second permission channel and leaves the model-facing sensitive-path classification unchanged.
