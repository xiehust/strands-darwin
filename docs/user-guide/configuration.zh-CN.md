...
auto 策略是持续授权，不是完成保证：每个候选还须通过持久化结算、驱动自然完成和有界语义完整性检查。`/clear`/`/rewind` 通过后台委派检查后，在第一次 await 前同步停止旧控制器自动工作，并持久化按来源会话保存的停止标记；即使检查点过期或构造失败，该会话仍须手动处理（对话和工具可用，cloud status 有提示）。这不撤销项目授权，未来后继会话的新回合仍继承它。HTTP 403 后重新授权会重置绑定旧 epoch 的停止锁存，但不清除较早的 held 条目；先明确 discard 该条目，后续同会话自动工作才可继续排空。每日预算不会提高 4096 个请求体/512 个 pending 的上限。满额拒绝的新发布仍可触发有限的已授权到期清理/发送，但被省略的候选不重试、不回填；仅超过七天的自动接受请求体可到期清理，手动请求体不删除。不新增配置字段或常驻任务。详见[自动上传约定](agentcore-memory.zh-CN.md#项目自动上传)。
