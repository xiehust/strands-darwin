...
只有授权后新产生的不可变 `darwin-upload-v2` 才可能自动上传。必须同时收到持久化回合结算和驱动自然完成的封口信号（先后顺序不限），然后发布事件文件及独立来源证明。仅观察到 `endTurn` 或 SDK 最终事件不够：驱动自然完成前被放弃或取消，候选仍须手动处理，即使存储的真实结束状态是 `endTurn`；不会改写该状态。正常结束的失败工具工作可上传，但 endTurn 不证明目标成功。取消、不完整、模型失败、严重采集错误、配对歧义须手动审核。必须有可追溯的实际操作结果及目标，或带原始身份的真实迟到结果；纯元数据、管理和后台接收确认不发送。

资格判断使用语义完整性标志及 **512 个身份键的结果义务账本**，独立于用于展示的 64 个操作正文和 96 条摘要。已检测到的源字段或结果不可用、身份歧义及未完成结果，不会随正文和摘要都被淘汰而消失。回合封口前，可靠匹配的最终结果即使在两者都被移除后，仍可恰好一次结清对应义务；派发回执不算完成。检测本身有界，不保证发现完整源内容中的所有问题。主动排除媒体、有界遍历或文本截断、原字节数未知，本身不否决 auto。不按敏感词、工具白名单或省略百分比否决，不扫描归档或恢复指针，不追溯授权旧事件。

默认每 UTC 日 **500 次尝试 / 104857600 字节（100 MiB）**，可经根级默认及项目覆盖调整。配额在发送前持久化预留，跨进程和重启共享（显式相同项目 ID 的工作副本也共享；账本位于 `~/.darwin/agentcore/auto-quota/<project-id>/usage.json`），按准确 HTTP 请求体序列化字节计数；取消、未知回执也保守占用，不等同于独立事件数或提取记录数。网络错误、429、5xx 最多三次同 token/同请求体尝试，等待 250/500 ms 可取消；IAM、配置 4xx 等永久失败将该授权停止并显示原因，修复后须重新确认。新的 `/cloud-memory auto` 授权会重置绑定旧 epoch 的 HTTP 403 停止锁存，但不会清除较早的 held 候选或同会话顺序屏障：先明确 discard 该 held token，后续自动任务才可继续排空。旧停止证据不改写。签名、凭证、停止文件读取和预留等待结束后，用共享配置锁覆盖重新读取策略和来源会话停止标记、验证以及同步调用 HTTP handler。原生命令撤销发布与请求启动按锁排序，已完成的撤销阻止后续启动。锁不等待网络响应，竞争命令立即拒绝而非无限等待；已发出的效果不能撤销，已收到回执仍保存。

发送在回合外进行，不阻塞 observer，不增加模型回合、TUI 行或常驻调度器。每轮最多处理八个候选，发送期间的新发布合并成后续一轮。已持久化的接受确认/回执及 discard tombstone，在 held 状态、顺序和名额计算之前跳过；即使手动接受与发送轮次竞争，已被手动接受的 token 也不占请求名额。同进程不同会话排在当前发送之后，不丢弃发布触发；跨进程争用仍由 outbox 锁拒绝，需要后续普通活动。预算、保留和顺序阻塞本身不会重启循环。同会话较早 pending 会阻塞后续，其他会话不受其饥饿影响。已启用控制器在新回合、模型切换、clear/rewind 和 status 边界读取本地新策略，包括尚无覆盖项的 manual/off；只有新回合采用新授权。策略损坏时关闭自动发送，不阻断普通回合。完全关闭云功能的 runtime 仍不创建控制器。预算暂停的事件保留供检查，在同一授权下后续普通活动时有限恢复。退出、取消及 clear 取消本 runtime 发送，清理等待最多两秒；退出不保证队列全部发完。AWS 接受事件不代表提取成功。

仅**自动接受的本地请求体**可在七天后、后续已授权活动中自动清理。pending、手动、失败请求体以及云记录不自动删除。持久化分区幂等回执先于删除并永久保留，不驱逐旧 256 条回执。每个范围最多 4096 个请求体、512 个待接受事件、32768 个目录项；256 个哈希分区各最多 4096 条回执。默认吞吐可容纳七天，更高预算可能先触及容量并明确暂停或省略新候选，不丢弃已有数据。新发布因请求体或 pending 满额而拒绝时，只要自动工作仍获授权且未取消，仍会触发一次有限的到期清理/发送轮次：可清理超过七天的自动接受请求体，或发送最多八个已有候选；被省略的新候选不重试、不回填。不增加容量自重试或常驻任务，手动请求体仍受保护。

`/cloud-memory pending [accepted] [after <64hex>]` 默认列出可处理请求体，包括 held 和 discard 清理中断的条目；`accepted` 切换到独立的已接受视图。两种视图都报告两类总数，每页最多 64 行，有后续内容时给出下一 token 游标。`after` 必须是 64 字符的小写十六进制 token。token 排序稳定，但不是快照；新加入且排序较早的 token 需要从头重新列出。独立 `darwin cloud-memory pending` 使用相同语法，仅本地只读，不访问网络、不写文件、不生成预览或授权证明。status 显示模式、项目覆盖、预算用量、排队/保留/暂停及失败。
...
| `/cloud-memory pending [accepted] [after <64hex>]` | 默认列出可处理条目（含 held/清理中断）；`accepted` 单独列已接受请求体；每页最多 64 条及下一游标。 |
...
- `/clear`、`/rewind` 通过后台委派检查后，在第一次 await 之前同步暂停旧控制器的 auto，再于构造后继会话前持久化按来源会话保存的 `*.session-stop.json`。后继控制器或重启不能自动排空该来源会话的旧 pending。即使 rewind 检查点过期或后继构造失败，也保守停止：旧对话和工具仍可用，但该会话保持手动处理，并有 cloud status 提示。项目 auto 策略不撤销，未来后继会话的新回合仍继承它。再次确认 auto 会明确提示需要新会话；刷新或重启不会隐藏旧来源会话的暂停状态。停止标记发布和其他发送器启动前的最终标记检查共用配置锁；来源会话停止不等同于整个授权 epoch 的传输失败。成功切换会重建控制器、刷新偏好，不清空持久队列。
- 切换或取消不能撤销已发出的 AWS 操作，已收到的接受确认仍保存。关闭时取消并等待本地工作最多两秒，不保证发完队列。删除原始事件或等待其 TTL 到期，**不会删除长期记录**。
