...
仍排除助手正文、推理和二进制/图像块，不遍历子代理对话，不读取工具路径指向的文件，也不补全 offload 或归档。工具公开文本中的文件内容、diff、日志、skill 或记忆现在可以保留。同步无 I/O 观察器在 trajectory 和 offload 截断前采集原始 SDK 内容，只有回合持久化结束后才排队写本地候选；无网络、无模型摘要或额外模型调用。旧请求体、证明和 token 保持原样，不重新生成，也不回填历史。
