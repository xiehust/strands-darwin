...
开启 `/cloud-memory auto` 后，Darwin 会把**当前项目中、授权后新产生且符合条件的回合内容，自动上传到 AgentCore Memory**。它不是同步整个聊天记录，也不是每轮额外调用模型写一份总结。

### 开启后的处理流程

#### 1. 保存当前项目的上传授权

前提是 AgentCore Memory 已配置启用，并且 trajectory 记录开启。在 TUI 空闲时输入：

```text
/cloud-memory auto
```

Darwin 会持久化一份绑定当前规范化工作目录和云端资源范围的授权，后续会话也可使用。它只覆盖授权后开始的新回合，**不回填历史，也不会顺手发送已有 pending**；不为其他项目授权，不重置当前对话，不调用模型。单独在全局配置写 `upload: auto` 不构成授权。

#### 2. 正常工作时，旁路采集上传材料

你继续让 Darwin 查代码、编辑文件、运行测试。采集器在主代理执行工具时记录材料，不参与代理决策，也不在采集回调中发起上传。

| 内容 | 具体是什么 |
| --- | --- |
| 用户目标（USER） | 本轮实际提交的文本，有长度限制。 |
| 工具操作（TOOL） | 工具名称、文本参数、对应结果，包括 bash 和任意 MCP 工具。 |
| 来源与状态（OTHER） | 会话、回合、操作顺序、结束状态，以及截断、缺失等说明。 |

不包含助手正文、推理内容、图片和二进制块，也不遍历子代理内部对话；子代理返回给主代理的公开报告可以作为工具结果被采集。不会为了上传额外读取工具参数中的文件路径，也不会从 offload 或历史归档中补全内容。

每个完整工具操作最多 **8 KiB**，新上传请求体最多 **96 KiB**，每个事件独占一个确定性的 Memory session 分片。旧请求体仍按原 256 KiB 上限读取与发送。超过容量的内容会截断或省略并标注，具体保留规则见[投影容量与质量](#投影容量与质量)。

> **这不是脱敏上传。** 工具参数和结果中的代码、文件内容、diff、日志甚至秘密都可能进入上传内容；不会靠敏感词过滤来保证保密。

#### 3. 等回合真正结束，再判断能否自动上传

回合结束记录持久化、驱动也给出结束判定后，才会生成不可变的本地候选请求，并判断自动上传资格。只有驱动自然完成本轮执行才可能自动上传；仅看到模型的 `endTurn` 不够。未自然完成的回合即使已持久化，也只能留下手动候选。

自动上传需要有实际操作及可追溯结果，并有用户目标，或能追溯到原始调用的后台迟到结果。取消、模型失败、未完成结果或严重采集／配对问题会使候选留待手动审核。只有文字问答、纯管理操作或只有后台派发确认而没有最终结果的回合，不符合自动上传条件。

**工具执行失败不一定阻止上传。** 例如“测试失败，然后定位问题并修复”也可以成为经验材料。这里判断的是记录是否完整、回合是否正常结束，不是认定任务成功。

#### 4. 回合外发送，检查配额、顺序并有限重试

候选先落入本地待发送队列，再由回合外的有限发送任务处理：

- 每轮发送任务最多处理 **8 个候选**。
- 默认每 UTC 日最多 **500 次尝试 / 100 MiB**，重试也占用配额。
- 同一会话按回合顺序发送；较早的 pending 可能挡住后续候选，需要手动发送或丢弃。
- 发送前重新检查授权、停止状态和配额。
- 网络错误、429、5xx 可重试，每个候选总共最多 **3 次尝试**，始终使用相同请求体和幂等 token。

预算用尽会暂停；IAM 等永久错误会停止该授权的自动发送并显示原因，修复后需要重新确认。没有常驻定时上传任务，也不会制造隐藏的模型回合。预算暂停的工作依赖后续普通活动再尝试，退出时不保证队列全部发完。

#### 5. AWS 接收事件，再异步生成长期记忆

发送使用 AgentCore 的 `CreateEvent`。`AWS event accepted` 只表示 **AWS 已接收事件**；后续由云端已配置的策略异步提取项目经验（episode）、反思（reflection）等记忆。

上传成功不等于已经生成长期记忆，刚上传后立即检索也可能没有结果。Darwin 不会在本地额外调用模型生成上传总结，也不会把“接收成功”当成“提取成功”。

本地会保存接收回执，防止重启后重复上传。只有**自动上传且已接受的本地请求体**可在七天后、后续授权活动中自动清理；这不删除云端长期记忆。

#### 6. 后续任务按需检索，而不是自动塞回所有历史

主代理可以按需调用 `episodic_recall` 检索类似任务的项目经验，或调用 `reflection_recall` 检索适用于当前情境的反思。这些仍是受权限控制的网络工具调用。开启 auto 不意味着每轮强制检索，也不意味着云端内容可以覆盖当前请求或项目规则。

跨项目用户偏好是另一条流程：即使云端提取出了偏好，也需要你 `inspect` 后明确 `confirm`，不会因为开启 auto 就自动采纳。本地 `/memory` 机制保持独立。

日常可用 `/cloud-memory status` 查看预算、排队及失败原因，用 `pending` 列候选、`preview <token>` 查看准确上传内容，用 `/cloud-memory manual` 停止尚未发送的自动工作。已发出的 AWS 操作不能撤销。完整语法见[日常命令速查](#日常命令速查)。
...
每个完整工具操作上限 8 KiB；新序列化 CreateEvent 请求体（包含 JSON 转义）上限 **96 KiB**，最多 100 条消息，每条不超过 100,000 UTF-8 字节。旧请求体读取与传输仍支持 256 KiB，事件文件仍支持 257 KiB；偏好证明、回执及其他状态仍为 64 KiB，非上传请求仍为 32,000 字节。

新事件各自使用 `darwin-part-<clientToken>` 作为 Memory session ID。token 继续绑定项目/资源范围、原 Darwin session、turn 和 closingSeq，恢复与重试不改变请求体或分片，不需要可变累计账本。原始会话出处、项目 namespace、授权和逻辑会话上传顺序不变。代价是不再在同一个 Memory session 内合并多个 turn；项目级检索与 reflection 范围仍共享。

96 KiB 是保守字节预算，**不是精确 token 数，也不保证避免限流**。AWS 的 episodic 配额是每 session 每分钟 50,000 tokens（固定），账户/Region 提取配额是每分钟 150,000 tokens（可提高），不是 session 生命周期总量限制。服务内部提示和账户并发仍可能触发限流。未分片的历史候选保持原请求体与证明，后续普通活动不再自动重试，pending 显示 held manual；仍可明确预览后手动发送，但这不会压缩旧输入。升级不重试失败提取任务、不回填或删除云数据。参见 [AWS Memory 配额](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/bedrock-agentcore-limits.html#agentcore-memory-service-quotas)。

最多保留八个临时回合，每回合 64 个完整操作、96 个省略正文的轻量摘要和 8 KiB 用户目标；另有最多八个待写任务；临时回合满额与待写任务满额分别计数，明确说明省略原因。采集窗口保留末尾 16 个操作及优先历史；较小的上传预算优先保留最后一个操作，再保留失败与紧随其后的恢复操作，然后保留近期历史，输出恢复原始顺序。pending/preview 分别显示目标、完整操作、缺失结果、内容截断、容量省略、内部事件计数和源内容限制，不把它们合并为一个误导的数字。没有目标也没有操作就不创建空元数据候选。
