...
扫描按文件系统枚举顺序，最多检查 128 个项目目录条目、合计 2,048 个会话目录条目；达到扫描上限时额外查看一个条目以判断是否还有遗漏。每份租约最多读 4,096 字节，再加一个溢出检测字节；最多显示 32 行有效租约，在已检查的子集中按项目 key、会话 ID 排序。跳过的条目和未显示的有效租约给出数量；未扫描的剩余条目标明数量未知。存储缺失或不可读会明确说明，不冒充完整空列表。死亡、其他主机、格式错误、过大、PID 无效、符号链接及不安全条目均被排除；OS 提供用户 ID 时还会检查文件和目录的归属。显示字段转为可打印 ASCII，每个字段最多 255 字符。
