...
`/rewind`（或在空闲且为空的输入框上连按两次 `Esc`）列出本会话已完成提示词的检查点——每条已完成的纯文本提示词在运行前都会留下一个不可变的 SDK 快照，每个会话最多 100 个——接受其中一个，就把对话分支到一个恢复到该边界的新后继会话；所选提示词回到编辑器但不发送，来源会话仍留在磁盘上、可恢复且逐字节不变。只有对话会移动：工作区文件、shell 与 `!` 的效果、hooks、MCP 写入、子代理、后台任务和已学习记忆都不会回滚，提示里会明说。
