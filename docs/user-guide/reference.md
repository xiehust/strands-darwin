...
- The terminal window/tab title (`terminalTitle`, default `true`) reads `darwin · <project basename> · <state>` with state exactly one of `idle`, `working`, `waiting for approval` or `N queued` — one OSC 2 sequence (`ESC ] 2 ; <title> BEL`) straight to stdout, only when stdout is a TTY and only when the composed title changes (a transition, never a tick), capped at 80 code points with control characters stripped from the project name; every exit path (`/exit`, `/quit`, Ctrl+C, Ctrl+D) restores the bare project name once, `/clear` keeps the same project and just continues; `-p` never writes one.
