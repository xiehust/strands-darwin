...
要审查指定提交而不是当前工作区，可用 `/review --commit <40-hex-SHA>`，例如 `darwin -p '/review --commit 0123456789abcdef0123456789abcdef01234567'`。固定提示要求按该提交与父提交的差异审查（合并提交取第一个父提交）；根提交则与空树比较，同时查看周围代码和相关测试。若 Git 对象缺失或不受支持，或有内容无法验证，必须如实说明，不得编造结果。只接受一个完整的 40 位十六进制 SHA，不得追加其他参数；以 `--commit` 开头但格式不对的调用在本地显示用法，不发起模型回合。其他 focus 文本仍按字面处理。

提示请模型阅读仓库指令、暂存和未暂存的改动、相关未跟踪文件及周围代码；按优先级报告可操作的缺陷，附上文件／行号证据和影响说明，并单列测试缺口。不要报告推测或仅涉及风格的问题；若没有发现可操作的缺陷，或有内容未检查、未验证（包括未运行的测试），都应如实说明。

它只是一个普通模型回合，不是独立审查执行器。提示要求未经另行请求不要修改文件或创建提交，但**这只是指引，不是强制只读模式**。`/review` 不切换权限模式，不做 shell／模板插值，也不自动启动子代理；工具调用仍由现有权限 gate 决定。需要强制限制时，请另外选择权限模式。

TUI 忙碌时照常排队，附上的剪贴板图片沿普通发送路径一起传递。TUI、dev-repl 和 text／JSON／stream-JSON 无交互驱动共用这次展开，例如 `darwin -p '/review 重点检查认证逻辑'`。轨迹保留实际提交的斜杠文本，不记录展开后的提示或图片字节。内置命令占用 `/review`，取代原来同名的自定义命令或 skill 斜杠调用；请把扩展改为 `audit` 等未保留名称。自定义命令冲突仍通过原有启动诊断报告；同名 skill 仍可用 `load_skill` 加载，但 `/review` 调用的是内置命令。
...
- `Ctrl+R` 搜索本项目已发送的提示词。输入文字筛选（不区分大小写），用 `Ctrl+R`/`Up`/`Down` 切换结果，按 `Enter`/`Tab` 把原文放回编辑器而不发送，按 `Escape` 原样恢复搜索前的草稿和光标。查询和每条候选预览各占一行：换行显示为 `⏎`，其他终端控制字符显示为转义文本。筛选和接受结果仍使用原文，不使用这些标记。`/rewind` 的候选预览也采用相同显示方式。
- `/` 会列出内置命令、skills 和自定义命令。菜单有行数上限，并始终围绕当前选中项显示，同时写明上下省略了多少项。
- `@` 异步扫描工作区，只插入路径文本。它不会打开文件或把内容塞进上下文；`.git`、`node_modules`、逃逸到仓库外的符号链接和超大扫描都会排除或截断。每个补全名称只占一个显示行：CRLF／LF／CR、VT／FF／NEL 及 Unicode 行／段分隔符显示为 `⏎`；其他 C0／DEL／C1 控制字符（包括制表符和 ANSI／OSC 转义字节）显示为字面文本 `\uXXXX`。匹配、排序及 Tab／Enter 接受仍使用原始路径。
- 路径放入草稿后，LF 仍换行，制表符仍占四格；其他控制字符（包括 CR 和 Unicode 行／段分隔符）显示为字面文本 `\uXXXX`，不会改变终端样式或布局。这只是显示方式，不会替换草稿原文：移动和删除仍按原始字素操作，破坏性编辑的撤销仍恢复原文与光标。行尾空间不足时，整个转义文本移到下一显示行。
...
- `Ctrl+R` 搜索或 `/rewind` 选择器打开时，粘贴会追加到查询，不修改草稿；查询上限为 256 个 Unicode 码点。与输入框一样，CRLF／CR 转为 LF，非文本控制字符被丢弃；粘贴的换行和制表符只是文本，不会接受结果或提交。只有单独按 `Enter`／`Tab` 才接受，按 `Escape` 恢复打开时的草稿和光标。权限框和压缩期间会先忽略粘贴，不传给搜索或输入框。
...
队列还有第二种行：**后台任务唤醒**。`bash start` 启动的任务结束时，一行 `queued · [task bg-xxxxxxxx succeeded] <command>` 进入队列（忙碌提示以 ` · 1 task wake` 计数），并像提示词一样出队——作为一个普通回合告知模型任务已结束，附带退出状态和输出尾部，模型无需再轮询 `wait`。它不属于你编辑：`Up` 取回和取消退回只把用户输入放回编辑器，唤醒条目留在队列中；`/clear` 会丢弃它们。若模型已在某个已完成回合中通过 `wait`/`status`/`list` 看到任务结束，则不会再发唤醒。在 `~/.darwin/config.json` 中设置 `backgroundTaskWake: false` 可只保留完成通知。
