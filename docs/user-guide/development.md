...
- **Turn numbers are unique within one trajectory file.** A resumed run continues from the highest `turn` in the file's tail (read the same way the last `seq` is), so `replay --turn N` names one turn. Records written before this landed, or after a tail with no readable record, can still contain several `turn 1` records; spend totals count actual closing records either way.
