...
`/setup-agentcore-memory [偏好说明]` 通过随包 skill 启动引导配置，完整指南在普通模型回合之前加载。不带参数也会启动；命令名精确匹配且不区分大小写，参数不是授权。Darwin 先通过权限检查只读查看现有 `~/.darwin/config.json`，运行离线 `darwin doctor`、本地 `darwin cloud-memory status` 和通过 SDK 有界只读检索的 `darwin cloud-memory preferences`。配置健康就跳过重复设置，不再询问 actor/默认值或要求重启；空记录算读取成功，不证明提取/写入。缺失或禁用后的配置分支才提问，检查受阻/失败则请求针对性修复，不自动重置。明确重新配置时，先报告原设置的健康结果，再确认具体修改。忙碌时排队到下一回合，trajectory 保留原始斜杠文本，补全和 `/help` 只列一次。与其他提示型命令一致，TUI、开发 REPL、文本及结构化无头 `-p` 都通过同一 runtime 路径展开它。没有回答时返回待确认问题，不宣称无人值守配置完成。见[配置指南](agentcore-memory.zh-CN.md)。

用户亲自提交的 TUI `/cloud-memory` 支持 `auto`、`manual`、`discard-legacy [<manifest-hash>]`、`status`、`preferences`、`list [preferences|episodes|reflections] [after <token>]`、`inspect <record-id>`、`confirm <record-id> <hash> global`、`forget <record-id>`、`delete <record-id> cloud`、`pending [accepted] [after <64hex>]`、`preview <token>`、`send <token> <preview-hash>`、`discard <token>`、`clear-accepted`。auto 仅持久化当前项目新回合授权，manual 立即取消未发送的自动任务。默认每 UTC 日 500 次尝试/100 MiB，自动接受的本地请求体保留七天，回执保留。根级 auto 不构成授权。旧格式清理先预览清单再提交哈希，只清理本地未接受、非 v2 请求体，不删除云记录。新投影保留原始 USER 目标，每个 TOOL 消息将调用身份、输入和对应结果配对，任意 MCP 与看似敏感的文本也不按名字过滤。pending/preview 显示目标是否存在、完整或缺失结果的操作数、内容截断、容量省略与源内容限制，不推断任务成功。原始 SDK 文本在 trajectory/offload 截断前采集，每个操作上限 8 KiB，新序列化请求体上限 96 KiB，每个事件独占一个确定性的 Memory session 分片。这是字节预算而非 token 保证。未分片历史事件仍按 256 KiB 上限读取与发送，但只允许明确手动发送，不再自动重试。不包含助手正文、推理或二进制/图像，不遍历子对话、不按路径读取或补全归档。内容可能含秘密，预览不等于保密保证。旧请求体、哈希和 token 不变。运行时使用官方 AWS SDK，不依赖可执行文件；旧 `agentCoreMemory.cliPath` 被忽略并显示迁移提示。独立 CLI `darwin cloud-memory` 只读，仅支持 `status`、`preferences`、`list`、`inspect`、`pending`、`preview`，不写批准凭据，不确认、发送或删除。失败、错误用法和取消返回非零退出码。哈希绑定内容而非人类身份；shell 权限不是沙箱。详见[配置、资源准备和安全边界](agentcore-memory.zh-CN.md)。


| pending 语法（TUI 或独立 CLI） | 只读结果 |
| --- | --- |
...
检查顺序：先检查当前规范化项目（TUI 的 runtime 项目根目录、CLI 的工作目录），TUI 还优先检查该项目的当前会话 ID；独立 CLI 没有当前会话身份。其余条目按文件系统枚举顺序检查。优先身份使用同一预算，即使缺失或不安全也占一个名额；枚举再次遇到时跳过，不重复读取或计数。上限不变：128 个项目条目、合计 2,048 个会话条目，每份租约 4,096 字节加一个溢出检测字节，显示 32 行，每个字段最多 255 个可打印 ASCII 字符。达到扫描上限时多看一个条目以检测遗漏；选定有限子集后才按项目 key、会话 ID 排序显示。排除的条目和未显示的有效租约给出数量，未检查的剩余部分数量未知。死亡、其他主机、格式错误、过大、PID 无效、符号链接和不安全条目均跳过，绝不清理。
...
| `Ctrl+S` | 暂存一份原样草稿／光标／附图；输入框为空时取回；拒绝覆盖和超过 65,536 码点的草稿，不自动发送（[保留与清空规则](using-darwin.zh-CN.md#草稿暂存)） |
| `y` / `n` / `Esc` | 回答权限框；Esc 表示拒绝；Ctrl+Y 不会批准 |
| `a` / `A` | 预览窄规则/整工具规则的完整 ASCII JSON；Enter 翻页，末页确认保存，`b` 返回；`y` 仍只放行本次，`n`/Esc 拒绝。窗口过小禁用保存，调整尺寸后从头预览 |
| `Ctrl+B` | 收起/展开工具详情 |
...
- 记录类型 `permissionDecision`（字段 `toolUseId`、`toolName`、`kind`、`risk`、`mode`、`source`、`outcome`、可选的 `rule`、`promptedUser`；绝不含工具输入）：权限 gate 每裁定一次工具调用，就在该回合内、紧挨在该调用的 `beforeToolCallEvent` 之前写入一条，并带上同一个 `toolUseId`。`outcome` 取值为 `write-scope-denied`、`deny-rule`、`plan-denied`、`yolo`、`safe`、`allow-rule`、`classifier`、`user-approved`、`user-denied`、`restart-limit-denied` 之一；`source` 为 `parent` 或子代理的 `<agent>#<dispatchId>`。`trajectory replay` / `/export` 只在 `promptedUser` 为 true 或结果为拒绝时打印一行 `permission · <tool> · …` 提示；`yolo`/`safe`/`allow-rule`/`classifier` 不打印任何内容。`trajectory search` 可匹配工具名、判定结果和规则；`trajectory list` 不变。只写日志：模型、工具结果、TUI 与无头输出均不变，也没有配置项。
