...
用户亲自提交的 TUI `/cloud-memory` 支持 `auto`、`manual`、`discard-legacy [<manifest-hash>]`、`status`、`preferences`、`inspect <record-id>`、`confirm <record-id> <hash> global`、`forget <record-id>`、`delete <record-id> cloud`、`pending [accepted] [after <64hex>]`、`preview <token>`、`send <token> <preview-hash>`、`discard <token>`、`clear-accepted`。auto 仅持久化当前项目新回合授权，manual 立即取消未发送的自动任务。默认每 UTC 日 500 次尝试/100 MiB，自动接受的本地请求体保留七天，回执保留。根级 auto 不构成授权。旧格式清理先预览清单再提交哈希，只清理本地未接受、非 v2 请求体，不删除云记录。新投影保留原始 USER 目标，每个 TOOL 消息将调用身份、输入和对应结果配对，任意 MCP 与看似敏感的文本也不按名字过滤。pending/preview 显示目标是否存在、完整或缺失结果的操作数、内容截断、容量省略与源内容限制，不推断任务成功。原始 SDK 文本在 trajectory/offload 截断前采集，每个操作上限 8 KiB，序列化请求体上限 256 KiB。不包含助手正文、推理或二进制/图像，不遍历子对话、不按路径读取或补全归档。内容可能含秘密，预览不等于保密保证。旧请求体、哈希和 token 不变。运行时使用官方 AWS SDK，不依赖可执行文件；旧 `agentCoreMemory.cliPath` 被忽略并显示迁移提示。独立 CLI `darwin cloud-memory` 只读，仅支持 `status`、`preferences`、`inspect`、`pending`、`preview`，不写批准凭据，不确认、发送或删除。失败、错误用法和取消返回非零退出码。哈希绑定内容而非人类身份；shell 权限不是沙箱。详见[配置、资源准备和安全边界](agentcore-memory.zh-CN.md)。
